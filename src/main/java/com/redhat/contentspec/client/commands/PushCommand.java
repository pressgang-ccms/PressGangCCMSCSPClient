...
public class PushCommand extends BaseCommandImpl {
    ...
    public void process(final RESTManager restManager, final ErrorLoggerManager elm) {
        ...
        try {
            success = csp.processContentSpec(contentSpec, getUsername(), logDetails, ContentSpecParser.ParsingMode.EDITED);
            if (success) {
        ...
        if (success && !pushOnly) {
            ...
            if (pushingFromConfig) {
                ...
                outputSpec = new File((cspConfig.getRootOutputDirectory() == null || cspConfig.getRootOutputDirectory().equals(
                        "") ? "" : (cspConfig.getRootOutputDirectory() + escapedTitle + File.separator)) + escapedTitle + "-post." + Constants.FILENAME_EXTENSION);
            } else {
